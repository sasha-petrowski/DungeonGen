...
public class RoomDungeon : Dungeon
{
    ...
    public override void Generate()
    {
        ...
        #region Create first and last rooms
        int startRoomWidth = Mathf.Min((Width - 2) / 2, MaxRoomWidth);
        int startRoomHeight = Mathf.Min((Height - 2) / 2, MaxRoomHeight);

        Room firstRoom = new Room(startRoomWidth, startRoomHeight);
        Room lastRoom = new Room(startRoomWidth, startRoomHeight);

        rooms.AddFirst(firstRoom);
        rooms.AddLast(lastRoom);
        #endregion

        #region Instantiate Rooms
        for (int i = 2; i < MaxRooms; i++)
        {
        ...
        if (! firstRoom.TryPlace(1, 1, this))
        {
            Debug.LogError("Can't place first room");
            return;
        }
        firstRoom.Index = 0;
        Rooms.Add(firstRoom);
        ...
        if (! lastRoom.TryPlace(Width - lastRoom.Width - 1, Height - lastRoom.Height - 1, this))
        {
            Debug.LogError("Can't place last room");
            return;
        }
        lastRoom.Index = 1;
        Rooms.Add(lastRoom);
